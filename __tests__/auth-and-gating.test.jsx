...
describe('sign-in page', () => {
  it('lists the learner accounts only — never the admin credential', async () => {
    render(<LoginPage />);
    expect(await screen.findByText('Demo credentials')).toBeTruthy();
    expect(screen.getByText(/learner@ledgerline\.in/)).toBeTruthy();
    expect(screen.getByText(/student@ledgerline\.in/)).toBeTruthy();

    // the staff credential must not leak onto the public page
    expect(screen.queryByText(/admin@ledgerline\.in/)).toBe(null);
    expect(screen.queryByText(/admin123/)).toBe(null);
  });
  ...
  it('fills email AND password when a credential card is clicked', async () => {
    ...
    expect(email.value).toBe('learner@ledgerline.in');
  ...
  it('rejects a wrong password and accepts the right one', async () => {
    ...
    await u.type(screen.getByPlaceholderText('you@firm.com'), 'learner@ledgerline.in');
    ...
    expect(read('user').email).toBe('learner@ledgerline.in');
    expect(read('user').role).toBe('user');
  });

  it('signs an administrator in from the staff page and sends them to /admin', async () => {
    const u = userEvent.setup();
    render(<AdminLoginPage />);

    expect(await screen.findByText('Staff credential')).toBeTruthy();
    await u.click(screen.getByText(/admin@ledgerline\.in/).closest('button'));
    await u.click(screen.getByRole('button', { name: /Sign in to the admin panel/ }));

    await waitFor(() => expect(read('user').role).toBe('admin'));
    expect(push).toHaveBeenCalledWith('/admin');
  });

  it('refuses a learner account on the staff page', async () => {
    const u = userEvent.setup();
    render(<AdminLoginPage />);
    await screen.findByText('Staff credential');

    await u.type(screen.getByPlaceholderText('you@ledgerline.in'), 'learner@ledgerline.in');
    await u.type(screen.getByPlaceholderText('••••••••'), 'learner123');
    await u.click(screen.getByRole('button', { name: /Sign in to the admin panel/ }));

    expect(await screen.findByText(/Staff credentials are required/)).toBeTruthy();
...
describe('course access gating', () => {
  ...
  it('lets an admin preview without buying', async () => {
    ...
    write('purchases', { 'admin@ledgerline.in': [] });
    render(<LearnView id="audit" />);
    expect((await screen.findAllByText(/Lesson 1 of 18/)).length).toBeGreaterThan(0);
  });
