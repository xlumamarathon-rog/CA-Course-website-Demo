...
export function useStored(key, fallback) {
  ...
  const set = useCallback((next) => {
    // functional updates apply to what is actually stored right now, not to a
    // React snapshot that may trail it by a render
    const current = typeof next === 'function' ? read(key, latest.current) : null;
